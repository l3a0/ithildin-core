...
**The premise is that a body change is irreversible for the consumer.**
Consuming repositories commit numbers computed from this code, into test
assertions, committed ledgers, and published documents. This package can be
re-read at any commit. A number already published from it cannot be
un-published. So reordering a sum or swapping one `numpy` call for another is
not a tidy-up, it is a re-pin of every consumer, and the asymmetry between
those two costs is what every ranking decision here appeals to.

Adding a function carries none of that cost, which is why new work is cheap
here and edits to existing bodies are not. [docs/design.md](docs/design.md)
...
This repo has three prose surfaces: this file, `README.md`, and
`docs/design.md`. Two things drift between them and the code.

1. **A default described in prose.** `README.md` says `ols` fits no implicit
   intercept and `adf_tstat` uses a fixed lag. Both are load-bearing and both
   are stated in the module docstrings too, so changing either moves three
   surfaces.
2. **A count of what is here.** This file, `README.md` and `docs/design.md`
   each say two modules ship. Adding a third moves all three.

There are no generated artifacts, no figure embeds, and no prose reference
that names a line number, so the wider sweep policy the template carries does
not apply yet. It becomes live the first time any of those three exists.
...
This repo holds no secrets and needs none. Nothing here reaches a network, a
vendor, or a filesystem path. Every function takes an array and returns a
number, which is a deliberate property rather than an accident of what has
been built so far: an estimator that reads a file or a configuration setting
cannot be re-derived from its arguments alone, and a consumer could then get
two answers from one call. The design doc's Considered and rejected register
carries that decision.

That means there is no configuration table to keep current. If a future module
needs one, it needs this paragraph rewritten first.
...
Neither optional policy module the template ships applies whole. Research pins
are for a repo that measures things, and this one computes rather than
measures. Cross-surface sweeps are for a repo with generated artifacts, and
this one has none. What follows is the policy this repo needs instead, and the
one rule it does take from the research-pins module is the first.
...
regression test here is re-pinned, the prose that quotes it moves in the same
change. Grep the rounded and spelled-out forms too.

### A change to a function body is a release, not a commit

The premise at the top says why. What it means in practice:

1. **Say which consumers it moves, in the pull request body.** Naming them is
   the only warning a consumer gets, because nothing in this repo can see
   their assertions.
2. **Tag the release.** A consumer pins an exact tag, so a body change that
   ships without one reaches nobody, which is worse than reaching them late.
3. **Bump the minor version for a new function and the patch version never.**
   There is no change here that is too small to move a consumer's number.

Adding a function is exempt from the first, since it moves nothing.

### A module joins this package when it has two consumers, not before

The bar is two repositories, not two call sites. A module with one consumer
belongs in that repository, where it keeps that project's vocabulary and
changes without a release.

Both modules here met the bar the same way and the evidence is worth keeping,
because it is what the bar means in practice.

- `timeseries` was already duplicated. The two copies parsed to the same tree
  once docstrings were set aside, which is the strongest form of the argument.
- `stats` had eleven consumers in `trading-strategies` and none in
  `quantitative-trading`, and moved because the second repository's next
  significance claim needs it and relocating exercised code is not the same as
  building something new.

Where a candidate fails the bar, say so on its issue rather than in a message,
so the next session inherits the answer.

### Every module names where it came from

A module copied from another repository names that repository, the commit it
was taken from, and what changed on the way over. A copy whose source is not
named cannot be diffed against the original, which throws away the cheapest
