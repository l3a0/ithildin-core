"""Regression and unit-root primitives for cointegration work.

A project that rolls its own least squares is a second implementation of the
calculation, and two implementations drift without either one looking wrong.
So the estimators live here once and the analyses call them:
...
copying past. Letting ``statsmodels`` choose the lag count can reverse a
cointegration verdict on the same data, because each added lag pulls the
statistic toward zero. A caller that wants a data-chosen lag is asking a
different question and should say so at the call site.

``ols`` fits no implicit intercept. Add a column of ones when one is wanted.
That keeps a with-intercept fit and a through-origin fit distinguishable where
the caller writes them, rather than hidden behind a flag, which matters
...
def ou_half_life(spread: NDArray[np.float64]) -> float:
    ...
    Regress the change ``d_z_t`` on the lagged level ``z_{t-1}``; the slope is
    ``-theta`` and the half-life is ``ln(2)/theta``. The unit is whatever the
    caller's step is, so a daily series gives trading days. Returns ``+inf``
    when the spread does not mean-revert, meaning a non-negative slope.
    """
